...
(function ($) {
    ...
    $.fn.centerBox = function () {
        ...
        let boxWidth = $selector.width();

        // calculate widths and positions
        let centeredLeft = (bodyWidth - boxWidth) / 2;
        let bodyMargin = windowWidth - bodyWidth;
        let left = bodyMargin / 2 + centeredLeft;
...
if (typeof Object.equals !== 'function') {
    ...
    Object.prototype.equals = function (object) {
        let thisProperties = Object.getOwnPropertyNames(this);
        let thatProperties = Object.getOwnPropertyNames(object);

        let thisLength = thisProperties.length;
        let thatLength = thatProperties.length;
        ...
        for (let property in this) {
            ...
            let type = Object.prototype.toString.call(this[property]).replace('[object ', '').replace(']', '');

            switch (type) {
                case 'Object':
                case 'Array':
                    let valid = this[property].equals(object[property]);
                    if (!valid) {
...
function generateString(chars) {
    ...
    let possible = 'qwertyuiopasdfghjklzxcvbnm QWERTYUIOPASDFGHJKLZXCVBNM9876543210.,;-';
    // cache the length of the possibilities
    let possibleLength = possible.length;

    // loop for the number of characters desired
    for (let i = 0; i < chars; i++) {
        // get a pseudo-random position of the possible characters and add to the string
 ...
 */
function screenSize() {
    let aHeight = window.screen.availHeight;
    let aWidth = window.screen.availWidth;
    let iWidth = window.innerWidth;
    let orientation = window.orientation;
    let retina = window.devicePixelRatio >= 2 ? 'Retina' : 'Not Retina';
    let higherDpi = iWidth > aWidth;
    let hiDpi = higherDpi || retina === 'Retina' ? 'High DPI' : 'Low DPI';
